...
const crosstalkSelect = (c, ctSelection, opts) => {
  ...
  ctSelection.on("change", (e) => {
    ...
    selected.forEach((sel) => {
      let found = c
        .getData()
        .findIndex((data) => data["CROSSTALK_KEYS"] == sel);
    ...
    c.views.map((v, vindex) => {
      v.geometries[0].elements.map(function (el, index) {
        if (indices.includes(index)) {
          opts.map((opt) => {
            if (opt.on !== "none") el.shape.attrs[opt.attribute] = opt.on;
          });
        } else {
          opts.map((opt) => {
            if (opt.off === "none")
              el.shape.attrs[opt.attribute] =
                originalAttrs[vindex].values[index];
            else el.shape.attrs[opt.attribute] = opt.off;
          });
        }
      });
    });
